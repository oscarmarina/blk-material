...
> [More info color-roles in Material](https://m3.material.io/styles/color/the-color-system/color-roles#8edc07d2-1579-4891-819f-b3a182704b19)

##### Setting Roles like CSS custom properties

For this purpose, the component will expose its various forms of visualization based on the roles, allowing another parent component to import them and adjust its visualization according to its roles for corresponding Contained Areas, including when it is nested within a component of the same type.
...
#### Understanding `roles values` composition:

Different combinations can be made between components.

1. A component has "role values" but doesn't contain another component with "role values."
2. A component has "role values" and also contain another component with "role values."
3. A component uses "role values" and nests within another instance of that component.

To cover all these cases, we always "compose" our "role values" and export them so that another component can compose them with its own or use them directly if it does not have its own.

> It will always be composed using CSS variables; should not be composed with CSS selectors.

1. **blk-button-tokens.js**
...
// roles values composition
// my own roles values and the roles values of my children
export const rolesComposition = concatenateStringsByKey([roles, blkButtonRolesComposition]);
```

3. **blk-xxxx-tokens.js**

```js
import { rolesComposition as blkButtonRolesComposition } from './blk-button-tokens.js';

// roles values composition
// Only the roles values of my children
export const rolesComposition = concatenateStringsByKey([blkButtonRolesComposition]);
```

**And the division between CSS variables and CSS selectors continues.**
...
> Even if a component has "role values," you can always use the ["component token"](#types-of-tokens-and-how-to-generate-them) to override the value.
